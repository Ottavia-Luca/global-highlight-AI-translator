...
- 流式翻译，逐字显示
- 中英双语互译（专有名词附带全称解释）
- SQLite 本地缓存，重复文字瞬间出结果（仅上限淘汰，不过期）
- 浮窗内二次划选翻译
- 收藏夹：浮窗内收藏，托盘右键打开，按日期筛选
- 托盘常驻，右键菜单收藏夹/开关/设置/退出
...
其余配置（API URL、模型、提示词、缓存上限等）可在 `.env` 中按需修改，也可通过托盘右键 → 设置界面修改。
